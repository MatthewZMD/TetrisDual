...
std::string cmdInterpreter(const std::string& cmd, const std::string cmdList[]);
 ...
 */
std::string cmdInterpreter(const std::string& cmd, const std::vector<std::string>& cmdList){
    ...
    if(newList.size() == 0){
        ...
        return cmdInterpreter(cmd.substr(1), newList);
    }
}
